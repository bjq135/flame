...
module.exports = class TagsService {
  ...
  async destroy(tagId) {
    ...
    let tagTitle = result[0].tag_title;
    sql = `SELECT COUNT(*) as counter FROM tb_article WHERE FIND_IN_SET(:title, tags)`;
    [result, fields] = await dbUtil.execute(sql, { title: tagTitle });
